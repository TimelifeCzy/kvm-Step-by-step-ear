...
static void vmcs_clear(struct vmcs *vmcs)
{
	...
	if (error)
		printk(KERN_ERR "kvm: vmclear fail: %p/%llx\n",
		       vmcs, phys_addr);
	printk(KERN_EMERG "[+]VMclear Success\n");
}
	...
	if (per_cpu(current_vmcs, cpu) != vcpu->vmcs) {
		...
		if (error)
			printk(KERN_ERR "kvm: vmptrld %p/%llx fail\n",
			       vcpu->vmcs, phys_addr);
		printk(KERN_EMERG "[+]Call VMptrld\n");
	}
...
static __init int alloc_kvm_area(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		printk(KERN_EMERG "[+]Alloc Init VMXON.revision_id\n");
...
static __init void kvm_enable(void *garbage)
{
	...
	printk(KERN_EMERG "[+]Call VMXON\n"); 
	printk(KERN_EMERG "[+]----------Entry VMX Root----------[+]\n");
}
...
static void enter_rmode(struct kvm_vcpu *vcpu)
{
	...
	// X86_EFLAGS_VM = 00010111
	// IOPL_MASK = 0011000000000000
	flags |= IOPL_MASK | X86_EFLAGS_VM;
...
#endif
	// 60000010: 01100000000000000000000000010000
	// 80050031: 10000000000001010000000000110001
	vmcs_writel(CR0_READ_SHADOW, cr0);
	printk(KERN_EMERG "CR0_READ_SHADOW: value %lx\n",cr0);
...
static void __set_cr4(struct kvm_vcpu *vcpu, unsigned long cr4)
{
	...
	printk(KERN_EMERG "CR4_READ_SHADOW: value %lx\n",cr4);
...
static void vmcs_write32_fixedbits(u32 msr, u32 vmcs_field, u32 val)
{
	...
	val &= msr_high;
	val |= msr_low;
	// if(MSR_IA32_VMX_EXIT_CTLS_MSR == msr) {
	// 	int clearn_bit = 1 << 2;
	// 	val &= ~clearn_bit;
	// }
	vmcs_write32(vmcs_field, val);
	printk(KERN_EMERG "vmcs_write32_fixedbits write buffer: value %lx\n",val);
}
...
#define SEG_SETUP(seg) do {					\
	...
	vmcs_writel(GUEST_RFLAGS, );
	// vmcs_writel(GUEST_RFLAGS, 0x246);
	...
	printk(KERN_EMERG "[+]VM_ENTRY_MSR_LOAD_COUNT %d\n", nr_good_msrs);
	...
	// 9bit
	// vm_entryctl_requested.fields.ia32e_mode_guest = true;	// x64 = 1 or x32 = 0
	// vm_entryctl_requested |= 516;
	vmcs_write32_fixedbits(MSR_IA32_VMX_ENTRY_CTLS_MSR,
                               VM_ENTRY_CONTROLS, vm_entryctl_requested);
	...
	printk(KERN_EMERG "[+]Entry MMU Init\n");
...
static int kvm_dev_ioctl_create_vcpu(struct kvm *kvm, int n)
{
	...
	printk(KERN_EMERG "[+]alloc vmcs memory\n");
	vmcs = alloc_vmcs();
	if (!vmcs) {
		mutex_unlock(&vcpu->mutex);
		goto out_free_vcpus;
	}
	printk(KERN_EMERG "[+]Init Vmcs.revision_id Success\n");
	vmcs_clear(vmcs);
	vcpu->vmcs = vmcs;
	vcpu->launched = 0;
	...
	printk(KERN_EMERG "[+]Entry vmcs working Init\n");
	...
	printk(KERN_EMERG "[+]Setup Vmcs Success & Create Vcpu Success\n");
	return 0;
...
static int handle_rdmsr(struct kvm_vcpu *vcpu, struct kvm_run *kvm_run)
{
	...
	case MSR_IA32_APICBASE:
		...
		break;
	case MSR_IA32_LSTAR:
		printk(KERN_EMERG "the process is\"%d\"(pid %i)\n",current->comm, current->pid);
		break; 
	default:
		if (msr) {
			data = msr->data;
...
{
	printk(KERN_EMERG "[+]Entry kvm_run\n");
	struct kvm_vcpu *vcpu;
	u8 fail;
...
again:
	...
	if (vcpu->irq_summary &&
	    !(vmcs_read32(VM_ENTRY_INTR_INFO_FIELD) & INTR_INFO_VALID_MASK)) {
			printk(KERN_EMERG "[+]entry kvm_try_inject_irq\n");
			kvm_try_inject_irq(vcpu);
		}


	if (vcpu->guest_debug.enabled) {
		printk(KERN_EMERG "[+]entry kvm_guest_debug_pre\n");		
...
static __init int kvm_init(void)
{
	...
	printk(KERN_EMERG "[+]cpu support vmx\n"); 
	...
	printk(KERN_EMERG "[+]enable by bios\n"); 
	kvm_init_debug();

	setup_vmcs_descriptor();
	// 申请vmxon
	r = alloc_kvm_area();
	printk(KERN_EMERG "[+] Init VMXon area\n"); 
	if (r)
		goto out;
	...
	printk(KERN_EMERG "[+] kvm_init success\n");
